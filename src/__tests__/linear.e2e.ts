...
test("single input, single output: f(x) = 5x + 4", () => {
...
test("single input, multi output: f(x) = [5x + 4, 0.2x - 7]", () => {
	const trainInputs = nest([-5, -3, -1, 1, 3, 5]);
	const trainOutputs = [
		[-21, -8],
		[-11, -7.6],
...
test("multi input, multi output: f(x, y) = [5x - 0.2y + 4, -0.2x + 3y - 8]", () => {
	const trainNs = [-5, -3, -1, 1, 3, 5];
	const trainInputs = flatten(trainNs.map(x => trainNs.map(y => [x, y])));
	const trainOutputs = trainInputs.map(([x, y]) => [5 * x - 0.2 * y + 4, -0.2 * x + 3 * y - 8]);
	const testNs = [-4, -2, 0, 2, 4];
	const testInputs = flatten(testNs.map(x => testNs.map(y => [x, y])));
	const testOutputs = testInputs.map(([x, y]) => [5 * x - 0.2 * y + 4, -0.2 * x + 3 * y - 8]);
	const specifications = [
		{
			kind: LayerKind.Input as const,
			width: 2,
